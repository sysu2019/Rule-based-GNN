...
def initilize_dataset(): 
	global candidate_heads,candidate_tails,gold_heads,gold_tails
	...
	for line in tool.read(args.train_file):
		...
		# 存储邻居
		glinks[t].add((r,h,0)) #存储头实体和关系标记为0
		glinks[h].add((r,t,1)) #存储尾实体和关系标记为1
			...
			if r in premise:
				...
				for r_, flag in conclusion:
					if flag == 1:
						...
						grelations[(t,h)].add(r_)
			gedges[t].add((r,h,0)) #存储头实体和关系标记为0
			gedges[h].add((r,t,1)) #存储尾实体和关系标记为1
		for e in gedges:
...
def train(args,m,xp,opt):
	...
	for positive, negative in generator_train_with_corruption(args):
		...
		N += len(positive)
		# print("N=",N)
		# print("loss=",float(loss.data)/len(positive))
		del loss
	return sum(Loss),N
...
def argument():
	...
	p.add_argument('--nn_model',		'-nn',  default='A0_candidate')
